...
# Take each frame
	...
	if k >= 49 and k <= 57:
		t = k - 48
	name = "/home/behnam/Downloads/Dubaisi/Station_10/SUV/suv-defect2.JPG"
	print name
	...
	hmin = check(cv2.getTrackbarPos('h_min', 'mask'), 124)
	hmax = check(cv2.getTrackbarPos('h_max', 'mask'), 211)
	smin = check(cv2.getTrackbarPos('s_min', 'mask'), 138)
	smax = check(cv2.getTrackbarPos('s_max', 'mask'), 218)
	vmin = check(cv2.getTrackbarPos('v_min', 'mask'), 137)
	vmax = check(cv2.getTrackbarPos('v_max', 'mask'), 202)
	# define range of blue color in HSV
