...
void findCircles(Mat img, vector<Vec3f> &circles, int minRadius, int maxRadius) {
    Mat gray;
    cvtColor(img, gray, CV_BGR2GRAY);
    HoughCircles(gray, circles, CV_HOUGH_GRADIENT,
                     2, 3 * maxRadius, 30, 20, minRadius, maxRadius);
}

int main() {
    Mat thresholded;
    // shape_feature_extractor sfe2(thresholded);
    for (int num = 3; num < 13; num++) { //loop for all the pictures
        //if (num != 3) continue;
        char address[40];
        ...
        for (size_t i = 0; i < sfe.size; i++) {
            ...
            if (ratio > .9 and ratio < 1.1 and rectangularity > .8) {
        ...
        // int maxRadius = mainThree[0].height / 6;
        // int minRadius = mainThree[0].height / 10;
        // Mat gray;
        // cvtColor(originalImg, gray, CV_BGR2GRAY);
        // HoughCircles(gray, circles, CV_HOUGH_GRADIENT,
        //              2, 3 * maxRadius, 30, 20, minRadius, maxRadius);
        // Mat circlesImage = Mat::zeros(img.size(), CV_8UC1);
        // circlesImage = circlesImage * 0;
        // int numTotalCircles = 0;
        // printf("what the fuck: %d\n", countNonZero(circlesImage));
        ...
        //         ,Scalar(0, 255,0),
        //         -1, 8, 0);
        //     if (circlesImage.at<uchar>(center) == 0) {
        //         circlesImage.at<uchar>(center) = 1;
        //         numTotalCircles++;
        //     }
        // }
        // printf("what the fuck: %d\n", countNonZero(circlesImage));
        ...
        vector<vector<Point>> grayRectangles, nice_contours;
        vector<Rect> blah;
        for (size_t i = 0; i < sfe2.size; i++) {
            RotatedRect rRect = sfe2.min_rects[i];
            Size2f size = rRect.size;
            Rect boundingRect = sfe2.bounding_rects[i];
            //****************************************************************************************************************************
            double sideRatio = size.height / size.width; sideRatio = sideRatio >= 1 ? sideRatio : 1 / sideRatio;
            if (!(sideRatio > 1.5 and sideRatio < 2.5)) continue;
            printf("side ratio %f:\n", sideRatio);
            //****************************************************************************************************************************
            double rectangularity = sfe2.good_contours_areas[i] / sfe2.min_rects_areas[i];
            if (!(rectangularity > .9)) continue;
            printf("rectangularity: %f\n", rectangularity);
            //****************************************************************************************************************************
            // double angle1 = 0, angle2 = 0, angleCos = 0;
            // angleCos = cos(rRect.angle / 180 * M_PI);
            // angle1 = findAngle(findCenter(mainThree[1]), rRect.center, findCenter(mainThree[0]));
            // angle2 = findAngle(findCenter(mainThree[2]), rRect.center, findCenter(mainThree[0]));
            // if (!(angle1 > 140 and angle2 > 140)) continue;
            // printf("angle: %f, %f\n", angle1, angle2);
            //****************************************************************************************************************************
            // double hull_ratio = utilities::calc_area(sfe2.hulls[i]) / (double) sfe2.min_rects_areas[i];
            // if (!(hull_ratio > .75 and hull_ratio < .85)) continue;
            // printf("hull ratio %f\n", hull_ratio);
            // printf("hull size: %ld\n", sfe2.hulls[i].size());
            //****************************************************************************************************************************
            //histogram analysis
            // Mat tmplt = imread("/home/behnam/workspace/Dubaisi/Data/51.png");
            // Mat roi(img, boundingRect);
            // double tmpltSideRatio = tmplt.rows / (double) tmplt.cols; tmpltSideRatio = tmpltSideRatio >= 1 ? tmpltSideRatio : 1 / tmpltSideRatio;
            // // area an histogram analysis
            // double histAnalysis = compareHistogram(tmplt, roi, 0);
            // if (histAnalysis < .1) continue;
            // printf("hist analysis: %f\n", histAnalysis);
            //****************************************************************************************************************************
            // circle analysis:
            Mat filledContourImage = Mat::zeros(img.size(), CV_8UC1);
            drawContours(filledContourImage, sfe2.good_contours, i, Scalar(100), -1);
            // int numAllCircles = 0, numCirclesInContour = 0;
            // Mat circlesImageRoi(circlesImage, boundingRect);
            // for (int x = 0; x < circlesImageRoi.cols; x++) {
            //     for (int y = 0; y < circlesImageRoi.rows; y++) {
            //         if (circlesImageRoi.at<uchar>(x, y) == 1) {
            //             numCirclesInContour++;
            //             //circle(img_clone, Point(x, y), 3, Scalar(0, 255, 0), -1, 8, 0);
            //         }
            //     }
            // }
            // Mat circleRoi(circlesImage, boundingRect);
            // Mat filledContourRoi(filledContourImage, boundingRect);
            // bitwise_and(circleRoi, filledContourRoi, circleRoi);
            // // int numCircles = numCirclesInContour;
            // int numCircles = countNonZero(circleRoi);
            // if (!(numCircles >= 5 and numCircles <= 7)) continue;
            // printf("num total circles: %d, num chosen circles: %d\n", numTotalCircles, numCircles);
            //*******************************************************************************************************************************
            //hsv thresholding:
            // Scalar lowerb(21, 85, 0);
            // Scalar upperb(24, 125, 255);
            // inRange(img_hsv, lowerb, upperb, thresholded);
            // Mat thresholdedRoi(thresholded, sfe2.bounding_rects[i]);
            // Mat filledContourRoi(filledContourImage, sfe2.bounding_rects[i]);
            // //thresholded = thresholded * 100;
            // printf("count non zero of the filled contour: %d\n", countNonZero(filledContourImage));
            // printf("count non zero of the roi of filled contour: %d\n", countNonZero(filledContourRoi));
            // printf("area of the contour: %f\n", utilities::calc_area(sfe2.good_contours[i]));
            // printf("count non zero of the thresholded image: %d\n", countNonZero(thresholded));
            // printf("count non zero of the roi of the thresholded image: %d\n", countNonZero(thresholdedRoi));
            // // imshow("hsv roi", thresholdedRoi);
            // // imshow("filledContourImage", filledContourRoi);
            // // bitwise_and(thresholded, thresholded, thresholded, filledContourImage);
            // printf("%d, %d, %d, %d\n", thresholdedRoi.rows, thresholdedRoi.cols, filledContourRoi.rows, filledContourRoi.cols);
            // int numQualified = 0, contourArea = 0, thresholdedArea = 0;
            // for (int x = 1; x < thresholdedRoi.cols - 1; x++) {
            //     for (int y = 1; y < thresholdedRoi.rows - 1; y++) {
            //         if (filledContourRoi.at<uchar>(x, y) != 0) {
            //              contourArea++;
            //             if (thresholdedRoi.at<uchar>(x, y) != 0) {
            //                 //thresholdedRoi.at<uchar>(x, y) = 1;
            //                 //printf("%d, %d\n", x, y);
            //                 numQualified++;
            //             }
            //         }
            ...
            //             //thresholdedArea++;
            //         // }
            //         //if (x >= 1140 and x <= 1150) printf("%d / %d, %d / %d\n", x, boundingRect.width, y, boundingRect.height);
            //     }
            // }
            // printf("afeter change:\n");
            // printf("count non zero of the thresholded image: %d\n", countNonZero(thresholded));
            // printf("count non zero of the roi of the thresholded image: %d\n", countNonZero(thresholdedRoi));
            // printf("real threshold area: %d\n", thresholdedArea);
            // printf("real contour area: %d\n", contourArea);
            // //thresholded = thresholded * 100;
            // printf("count non zero: %d\n", numQualified);
            // printf("****************************************\n");
            // // imshow("and  result", thresholdedRoi);
            // // waitKey(0);
            // double ratio = numQualified / (contourArea + .000000001);
            // if (!(ratio > .8)) continue;
            // printf("fill ratio: %f\n", ratio);
            //****************************************************************************************************************************
            double area = utilities::calc_area(sfe2.good_contours[i]);
            double area_ratio = area / mainThree[0].area();
            if (!(area_ratio < 2.5)) continue;
            printf("area: %f\n", area);
            printf("area ratio %f\n", area_ratio);
            //*******************************************************************************************************************************
            vector<Point> points = utilities::rect_to_contour(boundingRect);
            // vector<Point> points = sfe2.good_contours[i];
            grayRectangles.push_back(points);
            nice_contours.push_back(sfe2.good_contours[i]);
            blah.push_back(boundingRect);
            printf("*************************************************\n");
        }

        drawContours(img_clone, grayRectangles, -1, Scalar(0, 0, 100));
        drawContours(img_clone, nice_contours, -1, Scalar(0, 100, 100));
        imshow("image", img_clone);
        waitKey(0);
        // //destroyAllWindows();
        // for (size_t i = 0; i < blah.size(); i++) {
        //     Mat roi(img_clone, blah[i]);
        //     Mat circlesRoi(circlesImage, blah[i]);
        //     imshow("img", roi);
        //     printf("number of circles: %d\n", countNonZero(circlesRoi));
        //     waitKey(0);
        // }
        printf("************************************************************************************\n");
        //destroyAllWindows();
