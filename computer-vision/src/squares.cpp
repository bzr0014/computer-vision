...
{
	char mardas[] = "/home/behnam/Downloads/Dubaisi/";
	char raghas[40];
    help();
    ...
    for( int i = 0; i < 13; i++ )
    {
	sprintf(raghas, "%s%d.JPG", mardas, i);
        Mat image = imread(raghas, 1);
        if( image.empty() )
        {
            cout << "Couldn't load " << raghas << endl;
            continue;
        }
        GaussianBlur(image, image, Size(31, 31), 2, 2);
