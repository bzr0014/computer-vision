...
int main() {
	...
	vector<Vec4i> lines;
    HoughLinesP( img_canny, lines, 1, CV_PI/180, 20, 20, 10 );
    ...
    for (int i = 1; i < size; i=i+1) {
    	line(img, centerPoint, centers[i], Scalar(128, 128, 255));
    }

    //find the length of each line and print them above the line
    for (int i = 1; i < centers.size(); i++) {
    	double xDist = centerPoint.x - centers[i].x;
    	double yDist = centerPoint.y - centers[i].y;
    	double length = (int) sqrt(pow(xDist, 2) + pow(yDist, 2));
    	double actualLength = length / longestLineLength * REFERENCE_LENGTH;
    	Point textOrg = Point((centerPoint.x + centers[i].x) / 2 - 5, (centerPoint.y + centers[i].y) / 2 - 5);
        char text[15];
        sprintf(text, "%3.2f cm", actualLength);
    	putText(img, text, textOrg, FONT_HERSHEY_SCRIPT_SIMPLEX, .6, Scalar(0, 00, 250), 2, 3);
    }
